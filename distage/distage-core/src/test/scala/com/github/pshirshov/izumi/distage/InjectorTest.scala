...
//        make[Either[String, F[Int]]].from(Right(Pointed[F].point(1)))
    ...
    "support split and mixed-in tagless final modules" in {
      import Case20._
    ...
    "FIXME: Support [A, F[_]] type shape" in {
      ...
      abstract class Definition[C: Tag, R[_]: TagK: Pointed] extends ModuleDef {
        make[TestProvider[C, R]]
      }

      assert(new Definition[Int, List]{}.bindings.head.key.tpe == RuntimeDIUniverse.SafeType.get[TestProvider[Int, List]])
    }

    "FIXME: Support [A, A, F[_]] type shape" in {
      import Case20._

      abstract class Definition[A: Tag, C: Tag, R[_]: TagK: Pointed] extends ModuleDef {
        make[TestProvider0[A, C, R]]
      }

      assert(new Definition[Int, Boolean, List]{}.bindings.head.key.tpe == RuntimeDIUniverse.SafeType.get[TestProvider0[Int, Boolean, List]])
    }
    ...
    "progression test: No proper support for [A, F[_], G[_]] type shape - false positives generated by scala's type lambda generation during implicit search - leaking unresolved WeakTypeTags" in {
      ...
      abstract class Definition[A: Tag, F[_]: TagK, R[_]: TagK: Pointed] extends ModuleDef {
        make[TestProvider1[A, F, R]]
      }

      val fail = Try(assert(new Definition[Int, List, List]{}.bindings.head.key.tpe == RuntimeDIUniverse.SafeType.get[TestProvider1[Int, List, List]])).isFailure
      assert(fail)
    }
